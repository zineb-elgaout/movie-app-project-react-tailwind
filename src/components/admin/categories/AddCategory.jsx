...
const AddCategory = ({ onClose }) => {
  ...
  // Gestion du clic extérieur - Version finale
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (formRef.current && !formRef.current.contains(event.target)) {
        // Liste des éléments interactifs à exclure
        ...
        // Vérifier si l'élément cliqué ou ses parents sont interactifs
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Erreur lors de la création de la catégorie, voir console pour détails.");
    } finally {
      setIsSubmitting(false);
    }
  };
              ...
              <label className="flex flex-col w-full border-2 border-dashed border-gray-300 hover:border-pink-400 rounded-lg cursor-pointer transition-colors duration-200">
                <div className="flex flex-col items-center justify-center pt-5 pb-6 px-4">
                  ...
                  <p className="text-sm text-gray-500 text-center">
                    {formData.image ? formData.image.name : "Cliquez pour téléverser une image"}
                  </p>
                </div>
                <input 
                  type="file" 
