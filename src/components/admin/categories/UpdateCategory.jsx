...
const UpdateCategory = ({ onClose, category }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const updatedImage = formData.image instanceof File
    ...
    try {
      ...
      alert("Erreur lors de la mise à jour de la catégorie, voir console.");
    } finally {
      setIsSubmitting(false);
    }
  };
              ...
              <label className="flex flex-col w-full border-2 border-dashed border-gray-300 hover:border-pink-400 rounded-lg cursor-pointer transition-colors duration-200">
                <div className="flex flex-col items-center justify-center pt-5 pb-6 px-4">
                  ...
                  <p className="text-sm text-gray-500 text-center">
                    {formData.image ? (formData.image.name || "Image actuelle") : "Cliquez pour téléverser une image"}
                  </p>
                </div>
                <input 
                  type="file" 
