...
import { Users, Key, Folder, Film, Star, TrendingUp, Badge } from "lucide-react";
...
export default function Dashboard() {
  ...
  const [activeChart, setActiveChart] = useState('users'); // users, cartoons, favorites
  ...
  const getTimeSeriesData = () => {
    const keyMap = {
      month: { users: data.usersByMonth, cartoons: data.cartoonsByMonth, favorites: data.favoritesByMonth },
      week: { users: data.usersByWeek, cartoons: data.cartoonsByWeek, favorites: data.favoritesByWeek },
      year: { users: data.usersByYear, cartoons: data.cartoonsByYear, favorites: data.favoritesByYear },
    };
    ...
    <AdminLayout>
      <div className="flex flex-col flex-1 bg-gray-900 text-gray-200 min-h-screen">
        <main className="flex-1 pb-8 px-4 sm:px-6 lg:px-8 py-8">
          <Header
            header={{ prefix: "Admin", title: "Dashboard", subtitle: "Vue d'ensemble de votre application" }}
          />

          {/* Statistiques principales */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <StatCard title="Utilisateurs" value={data.totalUsers} icon={<Users size={24} />} />
            <StatCard title="Rôles" value={data.totalRoles} icon={<Badge size={24} />} />
            ...
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 shadow-lg border border-gray-700 lg:col-span-2">
              <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  {['users', 'cartoons', 'favorites'].map(type => (
                    <button
                      key={type}
                    ...
                    >
                      {type === 'users' ? 'Utilisateurs' : type === 'cartoons' ? 'Cartoons' : 'Favoris'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.byCategory}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#374151"/>
                    <XAxis dataKey="label" angle={-45} textAnchor="end" height={80} stroke="#9CA3AF"/>
                    <YAxis stroke="#9CA3AF"/>
                    <Tooltip />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 shadow-lg border border-gray-700">
              <h3 className="text-lg font-semibold mb-3 text-white">Détails par rôle</h3>
